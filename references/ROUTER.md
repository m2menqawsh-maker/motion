...
7. QC: probe_qc.py + final_qc.py حسب نوع الفيديو.
...
plan_gate.py (للتحقق من الخطة) • code_template_gate.py (لفحص أمان القوالب) • probe_qc.py (فحص الجودة التقني) • final_qc.py (الفحص النهائي) • materialize_project.py (بوابة نقل الميديا للبناء — scripts/)
...
استخدم أداة `ask_question` التفاعلية لطرح 5+ أسئلة جوهرية متعلقة بالـ VO (مثل الأبعاد ونمط الكابشن). ممنوع خطة أو كود قبل الأجوبة. المُخرَج: `00_answers.md`.
...
probe_qc.py + final_qc.py → `08_qc_report.json`.
