...
def test_every_active_template_has_disk_implementation(ground_truth_data):
    ...
    for item in active:
        path = ROOT / item["path"]
        if not path.exists() and not path.with_suffix(".tsx").exists() and not path.with_suffix(".ts").exists():
...
def test_template_aliases_file_in_sync_with_ground_truth():
    ...
    from scripts.generators.generate_template_aliases import generate_aliases
