...
Usage:
    python3 broll_layout_qc.py clip1.mp4 clip2.png --job-dir ${PLUGIN_DATA}/jobs/foo
...
def check_frame_overlap_pil(frame: Image.Image, caption_band: int, pip: tuple[int, int, int, int]) -> list[str]:
    issues = []
    gray = frame.convert("L")
    edges = gray.filter(ImageFilter.FIND_EDGES)
    global_stat = ImageStat.Stat(edges)
    global_edges = global_stat.mean[0]
    
    w, h = edges.size
    caption_zone = edges.crop((0, h - caption_band, w, h))
    caption_stat = ImageStat.Stat(caption_zone)
    caption_edges = caption_stat.mean[0] if caption_stat.count[0] > 0 else 0
    if caption_edges > max(global_edges * 1.5, 5.0):
        issues.append(f"Caption band has high visual detail (edge density {caption_edges:.1f} vs global {global_edges:.1f})")
        
    px, py, pw, ph = pip
    pip_zone = edges.crop((px, py, px + pw, py + ph))
    pip_stat = ImageStat.Stat(pip_zone)
    pip_edges = pip_stat.mean[0] if pip_stat.count[0] > 0 else 0
    if pip_edges > max(global_edges * 1.5, 5.0):
        issues.append(f"PiP zone has high visual detail (edge density {pip_edges:.1f})")
...
def review_asset(
    ...
    if path.suffix.lower() in VIDEO_EXTS:
        ...
        for idx, ts in enumerate(sample_times(duration, frames_per_video), start=1):
            ...
            frame_issues = check_frame_overlap_pil(frame, caption_band, pip)
            for iss in frame_issues:
                if iss not in issues: issues.append(iss)
    ...
    if override_reason and issues:
        import re
        has_fp = "false positive" in override_reason.lower()
        density_match = False
        for iss in issues:
            m = re.search(r"edge density (\d+\.\d+)", iss)
            if m and m.group(1) in override_reason:
                density_match = True
                break
                
        if has_fp and density_match and len(override_reason) >= 25:
            issues = [f"[OVERRIDDEN] {override_reason}"]
        else:
            issues.append(f"[OVERRIDE REJECTED] Override must contain 'false positive', be 25+ chars, and cite the exact 'edge density' value from the failure. Provided: {override_reason}")
...
def main() -> None:
    ...
    parser.add_argument("--overrides-file", default="", help="Path to JSON file mapping filenames to override reasons")
    ...
    try:
        ...
        overrides = {}
        if args.overrides_file and Path(args.overrides_file).exists():
            overrides = json.loads(Path(args.overrides_file).read_text(encoding="utf-8"))
        ...
        reviews = [
            review_asset(
                ...
                override_reason=overrides.get(p.name)
            )
            for p in assets
        ]
        ...
        for r in reviews:
            if r.get("issues"):
                all_issues.append(f"{Path(r['asset']).name}: {', '.join(r['issues'])}")
                real = [i for i in r['issues'] if not i.startswith("[OVERRIDDEN]")]
                if real:
                    unoverridden_issues.append(f"{Path(r['asset']).name}: {', '.join(real)}")

        status = "failed" if unoverridden_issues else "passed"
        ...
        report = {
            ...
            "reviews": reviews,
            "checklist": checklist,
            "next_action": "Automated layout check failed due to visual overlaps. Please crop or replace assets." if status == "failed" else "Open/read the contact sheet frames, mark each asset pass/crop-edit/re-render/replace before final composition.",
            "overlap_failures": all_issues
        }
