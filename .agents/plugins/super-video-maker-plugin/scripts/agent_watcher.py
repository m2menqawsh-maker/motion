...
def write_alerts(alerts_path, violations):
    ...
    with open(alerts_path, mode, encoding="utf-8") as f:
        for cat, rule, text in violations:
            ...
            f.write("---\n\n")
            print(f"⚠️ تم رصد مخالفة ({severity}): {rule.get('message', '')}")
...
def watch_transcript(project_id, brain_session_path):
    ...
    config = load_config()
    print(f"👀 بدء مراقبة الوكيل لمشروع {project_id}...")
            ...
            try:
                ...
                if data.get("type") == "USER_INPUT":
                    if os.path.exists(alerts_path):
                        ...
                        print("🧹 تم مسح التنبيهات السابقة بعد رسالة المستخدم.")
            except:
                pass

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("الاستخدام: python agent_watcher.py <project_id> <brain_session_path>")
        sys.exit(1)
    ...
    try:
        watch_transcript(project_id, brain_session_path)
