        ...
        if state.exists():
            try:
                ...
                if data.get("audio_free", False) and stage == "audio_analysis":
                    print("ℹ️ تجاوز فحص audio_analysis (مشروع صامت audio_free=true)")
                    return
        ...
        if state.exists():
            try:
                ...
                if data.get("audio_free", False):
                    print("ℹ️ تجاوز فحص الصوت (مشروع صامت audio_free=true)")
                    return
                if data.get("experimental", False):
                    print("ℹ️ تجاوز فحص الصوت (مشروع تجريبي)")
                    return
            except json.JSONDecodeError:
                pass
        ...
        if state.exists():
            try:
                ...
                if data.get("skip_backup", False):
                    print("ℹ️ تجاوز النسخة الاحتياطية (اختبار سريع)")
                    return
            except json.JSONDecodeError:
                pass
    ...
    def require_all_sentences_covered(self):
        ...
        if state.exists():
            try:
                ...
                data = json.loads(state.read_text(encoding='utf-8-sig'))
                if data.get("audio_free", False):
                    print("ℹ️ تجاوز مطابقة الجمل الصوتية (مشروع صامت audio_free=true)")
                    return
            except json.JSONDecodeError:
                pass
