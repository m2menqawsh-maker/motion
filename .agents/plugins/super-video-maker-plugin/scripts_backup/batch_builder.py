...
def build_scene(project_dir: Path, scene_idx: int, max_retries: int = 3) -> dict:
    delays = [0.5, 1.0, 2.0]
    ...
    for attempt in range(max_retries):
        try:
            ...
            if result["success"]:
                if attempt > 0:
                    print(f"✅ المشهد {scene_idx} نجح بعد {attempt + 1} محاولات")
                return {"status": "pass", "scene": scene_idx, "attempts": attempt + 1}
            
            if result.get("error_type") == "logical":
                print(f"❌ المشهد {scene_idx}: فشل منطقي، لا retry")
            ...
            if attempt < max_retries - 1:
                ...
                print(f"⚠️ المشهد {scene_idx}: خطأ عابر، retry بعد {delay}s...")
                time.sleep(delay)
            else:
                return {
                    "status": "fail",
...
def generate_main_composition(project_dir: Path, total_scenes: int, successful_scenes: list):
    ...
    if missing:
        print(f"⚠️ تحذير: المشاهد {sorted(missing)} فشلت")
        print("📝 MainComposition سيحتوي على المشاهد الناجحة فقط")
...
def main():
    ...
    print(f"📊 تقرير البناء: {report_path}")
