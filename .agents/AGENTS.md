...
## 3. دورة حياة المشروع والإجراءات الإلزامية

### عند بداية أي مشروع جديد (Initialization):
- نفّذ التجهيزات الأولية ونسخ ميديا المستخدم إذا لزم الأمر.
- شغّل `auto_backup.py` لأخذ نقطة استعادة مبدئية.
- شغّل `vo_quality_check.py` إذا كان هناك ملف صوتي مبدئي مقدم من المستخدم.

### عند فتح محادثة جديدة لمشروع قائم:
- **ممنوع** بدء العمل أو طرح اقتراحات من الصفر.
- شغّل فوراً `python scripts/session_manager.py resume <project_id>` واقرأ ملف `resume_brief.md` الناتج.
- هذا السكريبت سيقوم تلقائياً بتنظيف السجل (Transcript) وبدء المحادثة بسياق نظيف.
- لا تقترح أي قرار موجود في قائمة "القرارات المرفوضة" في `.session_state.json`.

### قبل كل خطوة في المشروع:
- شغّل `python scripts/selective_reader.py <phase>` لمعرفة الملفات المطلوبة وقراءتها.
- افحص وجود ملف `.agent_alerts.md` في مجلد المشروع، وتعامل مع التنبيهات إن وجدت قبل المتابعة.

### بعد كل مرحلة أو قرار مهم:
- شغّل `python scripts/context_compactor.py` (مثال: `set-phase`, `add-decision`, أو `compact`).

### قائمة "السكريبتات اليتيمة المحذوفة" (للتاريخ والتوثيق)
تم إجراء تدقيق شامل للنظام وتم حذف السكريبتات التالية لأنها كانت يتيمة أو تم استبدالها ببروتوكولات MCP حديثة (يُمنع محاولة استدعائها أو تخيل وجودها):
- `scripts/asset_gate.py`, `scripts/build_asset_index.py`, `scripts/migrate_assets.py`, `scripts/open_studio.py`, `scripts/render_project.py`
- `audit_imports.py`, `audit_skill.py`, `build_catalog.py`, `check_orphans.py`
- `stitch_skill.py`, `template_lint.py`, `verify_links.py`, `verify_media_layer.py`
- `generate_plan.py` (يُمنع استخدام أي سكريبت لكتابة الخطة الإبداعية).

## 4. البروتوكول الإلزامي والتعامل مع البوابات (Gates)
كل مهمة فيديو تمر عبر `rules/video-production-protocol.md` حرفياً (الإصدار 4.0).
البروتوكول يحتوي على 3 مراحل وبوابات صرامة:
...
بوابة الخطة `plan_gate.py` دورها الفحص والتحقق فقط.

قبل كتابة أي كود للمشاهد، يجب تخطي `stage_gate.py` بنجاح، وخلال البناء يتم استخدام `code_template_gate.py` و `motion_validator.py` لضمان مطابقة القوالب وحركة المشاهد لشخصية الحركة.
...
 # #   8 .   'D*H+JB  H'DA-5  'D4'ED  ( A u d i t   &   D o c s )  
 -   JH,/  *B1J1  A-5  4'ED  AJ  F I N A L _ A U D I T _ R E P O R T . m d   J9C3  -'D)  'DF8'E.  
 -   'DE3*F/'*  'D/'9E)  E*HA1)  AJ  E,D/  d o c u m e n t a t i o n /   ( E+D  S Y S T E M _ O V E R V I E W . m d   H  U S E R _ G U I D E . m d ) .  
 
## 8. التوثيق والفحص الشامل (Audit & Docs)
- يوجد تقرير فحص شامل في FINAL_AUDIT_REPORT.md يعكس حالة النظام.
- المستندات الداعمة متوفرة في مجلد documentation/ (مثل SYSTEM_OVERVIEW.md و USER_GUIDE.md).
