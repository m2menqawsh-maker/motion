...
> AUTO-GENERATED by build_ground_truth.py — DO NOT EDIT BY HAND
> Source: `tools/*.py + scripts/generators/materialize_project.py` | Generated: 2026-08-15T17:04:54
...
6. **المهارة قراءة فقط للميديا:** لا يُكتب ولا يُنسخ أي أصل داخل مجلد المهارة أبداً (remotion-template/public للعينات فقط). المصدر الوحيد = مجلدات دورة الحياة (${PLUGIN_DATA}/assets/${PLUGIN_DATA}/processed/storage)، والبناء يستلم الميديا حصراً عبر `scripts/generators/materialize_project.py`.
...
4. الأصول: `check_cache` → جلب عبر `media-sources-mcp` → بوابة ملاءمة → `save_to_cache` → والميديا تدخل البناء حصراً عبر `scripts/generators/materialize_project.py` (قاعدة 6).
...
1. حلل ملف التعليق الصوتي المرفوع **أولاً** عبر `audio-tools-mcp:analyze_voiceover`
   ...
   - استخرج النص الفعلي
2. طابق الوصفة: `python .agents/plugins/super-video-maker-plugin/tools/video_recipes.py match --goal "<الهدف>"`
...
1. `python scripts/generators/materialize_project.py projects/<id>`
2. اكتب كود Remotion مقفل على الـ Blueprint
3. `python scripts/validators/template_lint.py`
...
- `scripts/gates/validate_blueprint.py`: لفهم القواعد الصارمة لقبول الـ Blueprint (مثل ضرورة وجود مؤثرات صوتية لكل مشهد، والحد الأقصى لتكرار المؤثر الواحد، ومسارات الملفات الصحيحة).
