...
- `scripts/generators/build_asset_index.py` line 81 hardcodes its output path directly to `.agents/plugins/super-video-maker-plugin/ground-truth/ASSET_INDEX.json`.
- `scripts/gates/asset_gate.py` and `tools/mcp-servers/common-tools-mcp/utils/cache_ops.py` actively read from the plugin path.
...
| **`GENERATED_FROM_ROOT`** | **1** | `.agents/plugins/.../ground-truth/ASSET_INDEX.json` generated by `scripts/generators/build_asset_index.py`. |
...
- `.githooks/pre-commit` previously attempted auto-syncing ground-truth, but referenced `.agents/plugins/super-video-maker-plugin/scripts/generators/build_ground_truth.py`, which was moved to root in commit `9dcb888`. The hook silently exits without executing.
- `scripts/generators/build_asset_index.py` writes only to `.agents/plugins/.../ground-truth/ASSET_INDEX.json`, leaving root `ground-truth/ASSET_INDEX.json` out of sync.
...
1. **Current Test Suite (`tests/`):**
   - Root tests (`vitest`, `pytest`) exercise root implementations: `scripts/pipeline.py`, `scripts/generators/materialize_project.py`, `remotion-app/`, `templates/`, `contracts/`, `schemas/`.
...
- **The Single Exception:** Root `ground-truth/ASSET_INDEX.json` is a stale copy because `scripts/generators/build_asset_index.py` was hardcoded to output into the plugin directory.
...
1. **Fix Asset Index Inversion:** Modify `scripts/generators/build_asset_index.py` to write `ASSET_INDEX.json` to root `ground-truth/ASSET_INDEX.json`, and update `common-tools-mcp/cache_ops.py` and `scripts/gates/asset_gate.py` to read primarily from the root location.
...
4. **Repair Ground Truth Hook:** Update `.githooks/pre-commit` to point to `scripts/generators/build_ground_truth.py` in root instead of the non-existent plugin path.
